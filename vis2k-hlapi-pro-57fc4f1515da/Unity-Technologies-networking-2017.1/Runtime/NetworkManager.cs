...
namespace UnityEngine.Networking
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        public int numPlayers
        {
            get
            {
                int amount = 0; // vis2k: renamed to not be same as property name
                foreach (NetworkConnection conn in NetworkServer.connections) // vis2k: foreach
                {
                    if (conn != null) // vis2k: != instead of == and continue
                    {
                        amount += conn.playerControllers.Count(pc => pc.IsValid); // vis2k: Count instead of for loop
                    }
                }
                return amount;
            }
        ...
        // runtime data
        static public string networkSceneName = ""; // vis2k: this is used to make sure that all scene changes are initialized by UNET. loading a scene manually wont set networkSceneName, so UNET would still load it again on start.
        ...
        void Awake()
        {
            Debug.Log("Thank you for using HLAPI Pro! https://forum.unity3d.com/threads/unet-hlapi-pro-taking-unet-to-the-next-level.425437/"); // vis2k: little check message
            InitializeSingleton();
        }
        ...
        void OnValidate()
        {
            // vis2k: use clamp instead of two ifs
            m_SimulatedLatency = Mathf.Clamp(m_SimulatedLatency, 1, 500);
            m_PacketLossPercentage = Mathf.Clamp(m_PacketLossPercentage, 0, 99);
            m_MaxConnections = Mathf.Clamp(m_MaxConnections, 1, 32000);
        ...
        bool StartServer(MatchInfo info, ConnectionConfig config, int maxConnections)
        {
            ...
            if (m_CustomConfig && m_ConnectionConfig != null && config == null)
            {
                ...
                foreach (QosType channel in m_Channels) // vis2k: foreach
                {
        ...
        {
#if ENABLE_UNET_HOST_MIGRATION
            bool serverWasActive = NetworkServer.active; // vis2k: bool instead of var
#endif
            OnStopHost();
        ...
        void FinishLoadScene()
        {
            ...
            if (client != null)
            {
                // vis2k: process queued messages that we received while loading the scene
        ...
        public virtual void OnServerDisconnect(NetworkConnection conn)
        {
            ...
            // vis2k: Timeouts happen all the time, no need to throw an error.
            ...
            {
                // vis2k: sounds like the server disconnected, let's use a better message
        ...
        public Transform GetStartPosition()
        {
            ...
            s_StartPositions.RemoveAll(t => t == null); // vis2k: .RemoveAll instead of for loop
        ...
        public void SetMatchHost(string newHost, int port, bool https)
        {
            ...
            string prefix = https ? "https://" : "http://"; // vis2k: shorter
