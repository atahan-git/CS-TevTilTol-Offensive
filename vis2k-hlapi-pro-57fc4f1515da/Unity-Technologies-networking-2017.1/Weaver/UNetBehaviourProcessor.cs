...
namespace Unity.UNetWeaver
{
    class NetworkBehaviourProcessor
    {
        ...
        const int k_SyncVarLimit = 64; // vis2k: ulong = 64 bytes
        int m_QosChannel;
        ...
        */
        ...
        void GenerateSerialization()
        {
            ...
            foreach (FieldDefinition syncVar in m_SyncVars)
            {
                ...
                serWorker.Append(serWorker.Create(OpCodes.Ldc_I8, 1L << dirtyBit)); // vis2k: 8 bytes = long
        ...
        static void WriteDirtyCheck(ILProcessor serWorker, bool reset)
        {
            ...
            serWorker.Append(serWorker.Create(OpCodes.Callvirt, Weaver.NetworkWriterWritePacked64)); // vis2k
            if (reset)
            {
                serWorker.Append(serWorker.Create(OpCodes.Ldc_I4_1));
                serWorker.Append(serWorker.Create(OpCodes.Stloc_0));
        ...
        void GenerateDeSerialization()
        {
            ...
            VariableDefinition dirtyBitsLocal = new VariableDefinition(Weaver.int64Type); // vis2k
            serialize.Body.Variables.Add(dirtyBitsLocal);

            // get dirty bits
            serWorker.Append(serWorker.Create(OpCodes.Ldarg_1));
            //serWorker.Append(serWorker.Create(OpCodes.Callvirt, Weaver.NetworkReaderReadPacked32));
            serWorker.Append(serWorker.Create(OpCodes.Callvirt, Weaver.NetworkReaderReadPacked64)); // vis2k
            serWorker.Append(serWorker.Create(OpCodes.Stloc_0));
            ...
            foreach (FieldDefinition syncVar in m_SyncVars)
            {
                ...
                serWorker.Append(serWorker.Create(OpCodes.Ldloc_0));
                //serWorker.Append(serWorker.Create(OpCodes.Ldc_I4, 1 << dirtyBit));
                serWorker.Append(serWorker.Create(OpCodes.Ldc_I8, 1L << dirtyBit)); // vis2k
                serWorker.Append(serWorker.Create(OpCodes.And));
                serWorker.Append(serWorker.Create(OpCodes.Brfalse, varLabel));
        ...
        */
        ...
        MethodDefinition ProcessSyncVarSet(FieldDefinition fd, string originalName, long dirtyBit, FieldDefinition netFieldId) // vis2k
        {
            ...
            setWorker.Append(setWorker.Create(OpCodes.Ldc_I8, dirtyBit)); // vis2k: 8 byte integer aka long
        ...
        */
        ...
        FieldDefinition ProcessSyncList(FieldDefinition fd, long dirtyBit) // vis2k
        {
            ...
            foreach (FieldDefinition fd in m_td.Fields)
            {
                foreach (var ca in fd.CustomAttributes)
                {
                    if (ca.AttributeType.FullName == Weaver.SyncVarType.FullName)
                    {
                        ...
                        ProcessSyncVar(fd, 1L << dirtyBitCounter); // vis2k
                        dirtyBitCounter += 1;
                        numSyncVars += 1;
                ...
                if (Weaver.IsDerivedFrom(fd.FieldType.Resolve(), Weaver.SyncListType))
                {
                    ...
                    listFields.Add(ProcessSyncList(fd, 1L << dirtyBitCounter)); // vis2k
                    dirtyBitCounter += 1;
                    numSyncVars += 1;
