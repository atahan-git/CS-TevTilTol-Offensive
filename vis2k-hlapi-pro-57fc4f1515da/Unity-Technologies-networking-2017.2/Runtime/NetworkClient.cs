...
namespace UnityEngine.Networking
{
    public class NetworkClient
    {
        ...
        public static List<NetworkClient> allClients = new List<NetworkClient>(); // vis2k: public instead of get/set wrapper
        public static bool active { get; private set; } // vis2k: private set instead of variable + get/set wrapper

        public HostTopology hostTopology { get; private set; } // vis2k: private set instead of variable + get/set wrapper
        int m_HostPort;
        ...
        public string serverIp { get; private set; } // vis2k: private set instead of variable + get/set wrapper
        int m_ServerPort;
        int m_ClientId = -1;
        ...
        public int numChannels { get { return hostTopology.DefaultConfig.ChannelCount; } }
        public int hostPort
        {
        ...
        public bool ReconnectToNewHost(string serverIp, int serverPort)
        {
            ...
            m_Connection = null;
            m_ClientId = NetworkTransport.AddHost(hostTopology, m_HostPort);
        ...
        public bool ReconnectToNewHost(EndPoint secureTunnelEndPoint)
        {
            ...
            m_Connection.Initialize(serverIp, m_ClientId, m_ClientConnectionId, hostTopology);
            return true;
        }
        ...
        {
            // vis2k: use built-in method. the original one wasn't correct anyway.
            IPAddress temp;
        ...
        void PrepareForConnect(bool usePlatformSpecificProtocols)
        {
            ...
            if (m_UseSimulator)
            {
                int minTimeout = Mathf.Max((m_SimulatedLatency / 3) - 1, 1); // vis2k: use Mathf.Max to force >= 1
                int maxTimeout = m_SimulatedLatency * 3;

                if (LogFilter.logDebug) { Debug.Log("AddHost Using Simulator " + minTimeout + "/" + maxTimeout); }
                m_ClientId = NetworkTransport.AddHostWithSimulator(hostTopology, minTimeout, maxTimeout, m_HostPort);
            }
            else
            {
                m_ClientId = NetworkTransport.AddHost(hostTopology, m_HostPort);
        ...
        internal static void GetHostAddressesCallback(IAsyncResult ar)
        {
            try
            {
                ...
                client.serverIp = ip[0].ToString();
                client.m_AsyncConnect = ConnectState.Resolved;
                if (LogFilter.logDebug) { Debug.Log("Async DNS Result:" + client.serverIp + " for " + client.m_RequestedServerHost + ": " + client.serverIp); }
            }
        ...
        internal void ContinueConnect()
        {
            ...
            if (m_UseSimulator)
            {
                int simLatency = Mathf.Max(m_SimulatedLatency / 3, 1); // vis2k: use Mathf.Max to force >= 1
                ...
                m_ClientConnectionId = NetworkTransport.ConnectWithSimulator(m_ClientId, serverIp, m_ServerPort, 0, out error, simConfig);
            }
            else
            {
                m_ClientConnectionId = NetworkTransport.Connect(m_ClientId, serverIp, m_ServerPort, 0, out error);
            }
        ...
        void ConnectWithRelay(MatchInfo info)
        {
            ...
            m_Connection = (NetworkConnection)Activator.CreateInstance(m_NetworkConnectionClass);
            m_Connection.SetHandlers(m_MessageHandlers);
            m_Connection.Initialize(info.address, m_ClientId, m_ClientConnectionId, hostTopology);
        ...
        void GenerateError(byte error)
        {
            ...
            if (msgDelegate != null)
            {
                ...
                // vis2k: no more fixed error buffer size
        ...
        {
            // vis2k note: foreach doesn't seem to work here because something else accesses the list
            for (int i = 0; i < allClients.Count; ++i)
            {
